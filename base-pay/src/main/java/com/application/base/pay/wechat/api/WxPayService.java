...
public class WxPayService extends BasePayService {
    ...
    static Logger logger = LoggerFactory.getLogger(WxPayService.class.getName());
     ...
     */
    public WxPayService(PayConfig payConfigStorage) {
     ...
     */
    public WxPayService(PayConfig payConfigStorage, HttpConfig configStorage) {
    ...
    public boolean verify(Map<String, Object> params) {
        if (!"SUCCESS".equals(params.get("return_code"))){
            logger.debug(String.format("微信支付异常：return_code=%s,参数集=%s", params.get("return_code"), params));
            return false;
        }
        if(null == params.get("sign")) {
            logger.debug("微信支付异常：签名为空！out_trade_no=" + params.get("out_trade_no"));
            return false;
        }
        try {
    ...
    public JSONObject unifiedOrder(PayOrder order) {
        ...
        String requestXML = XmlUtils.getMap2Xml(parameters);
    ...
    public Map<String, Object> getParameter2Map(Map<String, String[]> parameterMap, InputStream is) {
        ...
        try {
            return XmlUtils.inputStream2Map(is, map);
        } catch (IOException e) {
    ...
    public <T> T refund(RefundOrder refundOrder, Callback<T> callback) {
        ...
        //设置签名
        setSign(parameters);
        return callback.perform(requestTemplate.postForObject(getUrl(WxTransactionType.REFUND), XmlUtils.getMap2Xml(parameters), JSONObject.class));
    }
    ...
    public <T> T downloadbill(Date billDate, String billType, Callback<T> callback) {
        ...
        //设置签名
        setSign(parameters);
        return callback.perform(requestTemplate.postForObject(getUrl(WxTransactionType.DOWNLOADBILL), XmlUtils.getMap2Xml(parameters), JSONObject.class));
    }
    ...
    public  <T> T secondaryInterface(Object transactionIdOrBillDate, String outTradeNoBillType, TransactionType transactionType, Callback<T> callback) {
        ...
        //设置签名
        setSign(parameters);
        return  callback.perform(requestTemplate.postForObject(getUrl(transactionType), XmlUtils.getMap2Xml(parameters) , JSONObject.class));
    }
