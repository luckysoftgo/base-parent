...
import com.application.base.pay.youdian.bean.YdPayError;
 ...
 *
 */
public class YouDianPayService extends BasePayService {
    
    protected static final Logger logger = LoggerFactory.getLogger(YouDianPayService.class);
...
//         4、将拼接出来的字符串连接上apb_nonce的值即AAAAAAAAAA。再连接  password=XXXXXXXusername=XXXXXAAAAAAAAAA
    ...
    public boolean verify(Map<String, Object> params) {
        if (!"SUCCESS".equals(params.get("return_code"))){
            logger.debug(String.format("友店微信支付异常：return_code=%s,参数集=%s", params.get("return_code"), params));
            return false;
        }
        if(params.get("sign") == null) {
            logger.debug("友店微信支付异常：签名为空！out_trade_no=" + params.get("out_trade_no"));}
        ...
        try {
            ...
            if ("401".equals(error.getErrorCode()) ||  "500".equals(error.getErrorCode())) {
                try {
                    ...
                    logger.debug(String.format("友店微信系统繁忙，(%s)ms 后重试(第%s次)", sleepMillis, retryTimes + 1));
    ...
    public YouDianPayService(PayConfig payConfigStorage) {
        super(payConfigStorage);
    }

    public YouDianPayService(PayConfig payConfigStorage, HttpConfig configStorage) {
