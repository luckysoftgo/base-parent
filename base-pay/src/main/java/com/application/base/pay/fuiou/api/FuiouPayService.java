...
public class FuiouPayService extends BasePayService {
     ...
     * 日志
     */
    protected final Logger logger = LoggerFactory.getLogger(FuiouPayService.class);
    /**
     * 正式域名
     */
     ...
     */
    public FuiouPayService (PayConfig payConfigStorage, HttpConfig configStorage) {
     ...
     * @param payConfigStorage 支付账户配置信息
     */
    public FuiouPayService (PayConfig payConfigStorage) {
    ...
    public boolean verify(Map<String, Object> params) {
        if (!"0000".equals(params.get("order_pay_code"))) {
            logger.debug(String.format("富友支付异常：order_pay_code=%s,错误原因=%s,参数集=%s", params.get("order_pay_code"), params.get("order_pay_error"), params));
            return false;
        }
        try {
