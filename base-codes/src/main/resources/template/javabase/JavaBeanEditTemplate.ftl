	  ...
	  $(function (){
	  		$("#propsName1").change(function(){
				...
				if(propsName1 != null && propsName1 != ''){
					$.ajax({
						url:'<%=request.getContextPath()%>/background/${firstLowerPoName}/findByName.html',
   ...
   <div class="box">
     <div class="box_border">
       <div class="box_top" ><b class="pl15" >系统管理——>修改信息</b></div>
		...
		<form action="<%=request.getContextPath()%>/background/${firstLowerPoName}/updateById.html" method="post" name="userForm" id="userForm">

			<input type="hidden" name="${tablePK}" id="${tablePK}" value=""><!-- 修改时候自动添加 -->
			...
			<table class="form_table pt15 pb15 list_table" width="100%" border="1" cellpadding="0" cellspacing="0">
				...
				<tr>
                    ...
                    <td class="td_left">
                       <sec:authorize ifAnyGranted="ROLE_${tableName}_edit">
