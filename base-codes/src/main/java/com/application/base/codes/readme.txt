...
1.
代码生成器对应的表名,必须要有"_"作为表的连接.eg: test_teacher,test_student;


2.
数据库设计上如果设计到逻辑删除，用 int 类型的disabled标识,还有就是必须有create_time字段用来标识创建时间。
...
create table table_name(
  id int(11) NOT NULL AUTO_INCREMENT COMMENT '自增长的主键', ##要是手动插入的主键,则用字符串标识,并在生成代码的generate.properties 中将 generate.primaryKeyStyle=MAN
...
4.
CommonConstant.java 中是代码生成中的常量的设置问题,需要在使用的时候注意更改文件的目录结构.
