...
import com.application.base.codes.javabase.constant.CommonConstant;
    ...
    private void initFilePath(){
        ...
        if(!BaseStringUtil.isEmpty(config.getSystemName())){
            ...
            if (!BaseStringUtil.isEmpty(config.getInstancePackage())) {
            	...
            	//Dao文件路径
                filePathMap.put(DAO_FILE_PATH,  interfaceProjectRoot +"dao"+Constants.Separator.SLASH + config.getInstancePackage()+Constants.Separator.SLASH + config.getPoName()+"Mapper" + Constants.FileType.JAVA_SUFFIX);
				...
				//Dao文件路径
                filePathMap.put(DAO_FILE_PATH,  interfaceProjectRoot +"dao"+Constants.Separator.SLASH + config.getPoName()+"Mapper" + Constants.FileType.JAVA_SUFFIX);
    ...
    public boolean createJavaBeanMapper(){
        if(filePathMap.containsKey(SQLMAP_FILE_PATH)){
            if(config != null && config.getParamMaps().size() > 0){
				...
				paramsMap.put("beanMapperPath",getJavaBeanDaoPath()+"."+config.getPoName()+"Mapper");
