...
public class MonGoCodeGenerateFactory {
	...
	private static void initCreateBean(){
		if (createBean==null) {
			...
			//设置数据库连接
			createBean.setMonGoInfo(mongoUrl, mongoUserName, mongoPassWord);
	...
	public static void codeGenerate(String tableName,String systemName, String codeName,
		...
		if (entityPackage.replace(str1, str2).contains(str2)) {
			basePackage = entityPackage.replace(str1, str2).replace(str2, ".").substring(0, entityPackage.length()-1);
