...
public class KylinObjPool<T> implements Cloneable  {
	...
	public KylinObjPool(final GenericObjectPoolConfig poolConfig, PooledObjectFactory<T> factory){
	...
	protected void closeInternalPool(){
		try {
			...
			throw new KylinException("Could not destroy the pool", e);
		}
	}
	...
	public T getResource(){
		try {
			...
			throw new KylinException("Could not get a resource from the pool", e);
		}
	}
	...
	private void returnResourceObject(final T resource){
		...
		try {
			...
			throw new KylinException("Could not return the resource to the pool", e);
		}
	}
