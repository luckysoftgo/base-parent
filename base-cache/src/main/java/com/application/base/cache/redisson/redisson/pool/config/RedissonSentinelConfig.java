...
public class RedissonSentinelConfig extends RedissonBasicConfig implements Serializable {
	...
	public Config getInstance(){
		...
		config.useSentinelServers().setMasterConnectionPoolSize(getMasterConnectionPoolSize());
		//设置对于slave节点的连接池中连接数最大为500
		config.useSentinelServers().setSlaveConnectionPoolSize(getSlaveConnectionPoolSize());
		//如果当前连接池里的连接数量超过了最小空闲连接数，而同时有连接空闲时间超过了该数值，那么这些连接将会自动被关闭，并从连接池里去掉。时间单位是毫秒。
		config.useSentinelServers().setIdleConnectionTimeout(getIdleConnectionTimeout());
		//同任何节点建立连接时的等待超时。时间单位是毫秒。
		config.useSentinelServers().setConnectTimeout(getConnectTimeout());
		//等待节点回复命令的时间。该时间从命令发送成功时开始计时。
		config.useSentinelServers().setTimeout(getTimeout());
		config.useSentinelServers().setPingTimeout(getPingTimeout());
		//当与某个节点的连接断开时，等待与其重新建立连接的时间间隔。时间单位是毫秒。
		config.useSentinelServers().setReconnectionTimeout(getReconnectionTimeout());
